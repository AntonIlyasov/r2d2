...
CamControl::CamControl(std::string tcp_ip_save_frame, int tcp_port_save_frame)
  ...
  toSaveVideoBashPub = _node.advertise<std_msgs::Int32>(TO_SAVE_VIDEO_BASH_TOPIC_NAME, 0);
...
void CamControl::checkSaveVideo(){
  ...
  /* отправляем сообщение в топик на запись видео */
  std_msgs::Int32 msg;
  msg.data = 1;
  toSaveVideoBashPub.publish(msg);
  ...
  while(ros::ok()){
    ros::spinOnce();
    if(prev_cmd == SAVE_VIDEO_CMD && prev_cmd != dataFromVicTcpRx[0]){
      msg.data = 0;
      toSaveVideoBashPub.publish(msg);
